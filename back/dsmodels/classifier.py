    ...
    def __init__(self, num_classes, lr=0.005, max_iter=200, min_iter=2, min_dloss=0.0001, optim="adam", lossfn="MSE",
        ...
        if self.device.type == "cuda" or self.device.type == "mps":  
            if not torch.cuda.is_available() and not torch.backends.mps.is_available():
                #yield "GPU acceleration is not available, using CPU"
    ...
    def _optimize_debug(self, X, y, optimizer, criterion, print_init_model=False, print_final_model=False, print_time=True,
        ...
        for epoch in range(self.max_iter):
            ...
            for Xi, yi in train_loader:
                ...
                if print_epoch_progress:
                    acc_n += ni
                    yield ("\r %d%% [" % (100*acc_n/N)) + "#"*int(25*acc_n/N) + " "*int(25 - 25*acc_n/N) + "]"
                tq = time.time()
                optimizer.zero_grad()
                y_pred = self.model.forward(Xi)
        ...
        if print_partial_time:
            yield "├- Forward eval time:  %.3fs" % dt_forward
            yield "├- Loss backward time: %.3fs" % dt_loss
            yield "├- Optimization time:  %.3fs" % dt_optim
            yield "└- Normalization time: %.3fs" % dt_norm
        ...
        if return_partial_dt:
            ...
            yield f"Losses: {losses}, Epoch: {epoch}, Time: {dt}, Forward: {dt_forward}, Loss: {dt_loss}, Optim: {dt_optim}, Norm: {dt_norm}"
        else:
            #return losses, epoch, dt
            yield  f"Losses: {losses}, Epoch: {epoch}, Time: {dt}"
