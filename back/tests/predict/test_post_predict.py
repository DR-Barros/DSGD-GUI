...
def test_post_predict(client, tmp_path):
    df = pd.DataFrame({
    ...
    # Realizar una predicción usando el modelo subido
    predict_data = [[1, "A"], [2, "B"], [3, "C"]]
    response = client.post(
        f"/dsgd/api/predict/{experiment['id']}",
        json={"predictData": predict_data}
    )
    assert response.status_code == 200
    res = response.json()
    assert "predictions" in res
    assert len(res["predictions"]) == len(predict_data)
    assert "labels" in res
    assert set(res["labels"]) == {'0', '1'}
    assert "probabilities" in res["predictions"][0]
    assert "class" in res["predictions"][0]
    assert "rules" in res["predictions"][0]
    
    
def test_post_predict_no_data(client, tmp_path):
    df = pd.DataFrame({
        "feature1": [1, 2, 3, 1, 1, 2, 3, 1, 2, 3],
        "feature2": ["A", "B", "C", "A", "A", "B", "C", "A", "B", "C"],
        "target": [0, 1, 0, 0, 0, 1, 0, 0, 1, 0]
    ...
    with open(model_path, "rb") as f:
        ...
        assert response.status_code == 200
    response = client.post(
        f"/dsgd/api/predict/1",
        json={"predictData": []}
    )
    assert response.status_code == 404
    res = response.json()
    assert res["detail"] == "No data provided for prediction"
...
def test_post_predict_no_iteration(client):
    response = client.post(
        f"/dsgd/api/predict/9999",
        json={"predictData": [[1, "A"], [2, "B"], [3, "C"]]}
    )
    assert response.status_code == 404
    res = response.json()
    assert res["detail"] == "Iteration not found"