...
def test_delete_iteration(client, tmp_path):
    df = pd.DataFrame({
    ...
    #borramos la iteracion
    response = client.delete(f"/dsgd/api/experiments/iteration/1")
    assert response.status_code == 200
    assert response.json()["detail"] == "Iteration deleted"
    #comprobamos que no hay iteraciones
    response = client.get(f"/dsgd/api/experiments/iteration/{experiment['id']}")
    assert response.status_code == 200
    iterations = response.json()
    assert len(iterations) == 0
    #comprobamos que no se pueden eliminar iteraciones que no existen
    response = client.delete(f"/dsgd/api/experiments/iteration/1")
